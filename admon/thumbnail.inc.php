...
class Thumbnail {
	 ...
	 * @param int $cropSize
	 */
	function cropFromCenter($cropSizeWidth,$cropSizeHeight) {
	    if($cropSizeWidth > $this->currentDimensions['width']) $cropSizeWidth = $this->currentDimensions['width'];
	    if($cropSizeHeight > $this->currentDimensions['height']) $cropSizeHeight = $this->currentDimensions['height'];

	    $cropX = intval(($this->currentDimensions['width'] - $cropSizeWidth) / 2);
	    $cropY = intval(($this->currentDimensions['height'] - $cropSizeHeight) / 2);

	    if(function_exists("ImageCreateTrueColor")) {
			$this->workingImage = ImageCreateTrueColor($cropSizeWidth,$cropSizeHeight);
		}
		else {
			$this->workingImage = ImageCreate($cropSizeWidth,$cropSizeHeight);
		}
		...
		$this->oldImage = $this->workingImage;
		$this->newImage = $this->workingImage;
		$this->currentDimensions['width'] = $cropSizeWidth;
		$this->currentDimensions['height'] = $cropSizeHeight;
	}
	 ...
	 */
	function createReflection($percent,$reflection,$white,$border = true,$borderColor,$red,$green,$blue) {
        ...
        $colorToPaint = ImageColorAllocateAlpha($this->workingImage,$red,$green,$blue,0);
        ...
        for($i=0;$i<$reflectionHeight;$i++) {
            $colorToPaint = imagecolorallocatealpha($this->workingImage,$red,$green,$blue,($i/$reflectionHeight*-1+1)*$white);
