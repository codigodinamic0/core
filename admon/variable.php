...
<?php
	...
	crear_json_vistas("variable","order by id_variable");
?>
<?php
$id=$_GET['id'];
$idr=$_GET['idr'];
...
if ($op=="1") { 
	...
	$sql = $sql . ", script_variable= '" . sql_seguro(base64_encode($_POST['script_variable'])) . "'";
...
<? if ($op=="2") {
	$sql = "INSERT INTO $t1(codigo_variable, tipo_variable, nombre_variable,valor_variable, script_variable, contenido_variable) values ";
	...
	$sql = $sql . " ,'" . sql_seguro(base64_encode($_POST['script_variable'])). "'";									 
...
<p class="punteado"><strong><?php $valor=variable(8,2); echo $valor[0]; ?>  <?php echo $_SESSION['nombre_usuario'] ?></strong></p>
            ...
            <h2><?php if($id){$valor=variable(2,2); echo $valor[0];} else{$valor=variable(3,2); echo $valor[0];}?> <span class="urgente"><?php echo $modulo ?> 
            	<?php 
					...
					$row = $db->fetch_assoc();
				?> 
<?php echo $row["nombre_variable"]?></span></h2>
              </div>
			  <div class="clear"></div>
			  <!-- End .clear -->
		  ...
		  <div class="clear"></div> <!-- End .clear -->
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-header">
					...
					<h3 class="img_conf"><?php echo $modulo ?></h3>
...
if($row["codigo_variable"]=="")
{
			$db->select("variable", "codigo_variable", "where tipo_variable='$_GET[idr]' order by codigo_variable DESC");
			$num = $db->num_rows();
			...
			{ 
				$db->select("variable", "codigo_variable", "where tipo_variable='$_GET[idr]' and codigo_variable=$i order by codigo_variable DESC");
				$numn = $db->num_rows();
				if($numn==0)
				{
...
?>
									...
									<input name="codigo_variable" type="text" class="text-input small-input"  id="codigo_variable" value="<?= $codigo?>" size="32" readonly />
</p>
...
<input name="contenido_variable" type="text" class="text-input large-input" id="contenido_variable" value="<?= $row["contenido_variable"]?>" />
		...
		<p>
				<label>Script  <?php echo $modulo ?></label>
				<textarea name="script_variable" cols="50" rows="5"  id="script_variable"><?= base64_decode($row['script_variable']) ?></textarea>
			</p>						
