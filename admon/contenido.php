...
<?php
	function pintar_categorias(&$arreglo, $nivel_cat, $nivel_categoria_final){
		...
		foreach ($arreglo as $row){
			...
			/*buscar cantidad productos*/
			$db->select("matrix","COUNT(id_matrix) total_reg","WHERE id_categoria = '".$row['id_categoria']."'");
			$row_reg = $db->fetch_assoc();

			$padding_left = $nivel_cat*20;
			$html = '<tr class="row-cat row-cat-'.$row['nivel_categoria'].'">
						<td valign="top" style="padding: 10px 10px 10px '.$padding_left.'px;" class="tree-icon tree-icon-'.$row['nivel_categoria'].'">
							<a href="contenido.php?idr='.$_GET['idr'].'&amp;nivel='.$nivel_categoria_final.'&amp;id_categoria='.$row['id_categoria'].'" title="'.$row['nombre_variable'].'" class="txt_ingresar">
							'.$row['nombre_categoria'].'('.$row['id_categoria'].' - '.$row_reg['total_reg'].')';
			if($row['img'] <> ""){
			...
			if($row['nivel_categoria'] == $nivel_categoria_final){
				...
				<i class="fa fa-plus" style="color: #57a000;" aria-hidden="true"></i> Agregar o listar '.$row['nombre_categoria'].'</a>';
			}elseif(sizeof($row['hijos']) <= 0){
				$html .= '<a class=Ntooltip href="contenido.php?idr='.$_GET['idr'].'&amp;nivel='.$nivel_categoria_final.'&amp;id_categoria='.$row['id_categoria'].'" title="">
				<i class="fa fa-plus" style="color: #57a000;" aria-hidden="true"></i> Agregar o listar '.$row['nombre_categoria'].'</a>';
			}
			$html .= '</td>
