...
if ($op=="1") { 
	...
	$sql = "UPDATE $t1 SET idr='" . sql_seguro($_POST['idr']) . "'" . 
	        ", nombre_l= '" . sql_seguro($_POST['nombre_l'])  . "'" . 
			", url= '" . sql_seguro($_POST['url']) . "'" . 
			" WHERE id_l= '" . sql_seguro($_POST['id']) . "'";
...
<? if ($op=="2") { 
	$sql = "INSERT INTO $t1(idr, nombre_l, url) values " . 
			" ('" . sql_seguro($_POST['idr']) . "'" . 
			" ,'" . sql_seguro($_POST['nombre_l']) . "'" . 
			" ,'" . sql_seguro($_POST['url']) . "')";
	//echo $sql;
...
<? if ($op=="3") { 
	...
	$db->delete($t1,"where id_l='" . $_GET['idb'] . "'");

	
	?>
   
<script>window.location="<?= $_SERVER['PHP_SELF']?>?idr=<?php echo $idr ?>";</script>
...
$db->select($t1,"*","where id_l='" . $_GET['id'] . "'");
$row = $db->fetch_assoc();

?> <?= $row["nombre_l"]?> -- <?php echo $papa ?> </span></h2>
              </div>
			  <div class="clear"></div>
			  <!-- End .clear -->
              ...
              <div class="notification attention png_bg">
				<a href="#" class="close"><img src="imgs/cross_grey_small.png" title="Close this notification" alt="close" /></a>
				<div>
                  <?php $valor=variable(64,2); echo $valor[1]; ?></div>
			  </div>
              </div>
...
<p>
									...
									<input name="nombre_l" type="text" class="text-input small-input"  id="nombre_l" value="<?= $row["nombre_l"]?>" size="32" />
</p>
...
<p>
									<label>
                                    <?php $valor=variable(65,2); echo $valor[0]; ?>
			 </label>
            <input name="url" type="text" class="text-input small-input"  id="url" value="<?= $row["url"]?>" size="32" />
</p>
	  ...
	  <p>
									<input class="button" type="submit" value="<?php if($id){$valor=variable(2,2); echo $valor[0];} else{$valor=variable(3,2); echo $valor[0];}?> <?php echo $modulo ?>" /> <input name="id" type="hidden" id="id" value="<?= $row["id_l"]?>" />
...
<td><a href="#" title="title" ><span class="txt_verde">
                                   <?php $valor=variable(65,2); echo $valor[0]; ?>&nbsp;<?php echo $modulo ?>
                                  </span></a></td>
...
$inicio = ($pagina - 1) * $TAMANO_PAGINA; 
                    ...
                    $db->select("menu_r","*"," where idr='".$_GET['idr']."'");
                    $num_total_registros = $db->num_rows();
                    //calculo el total de p&aacute;ginas 
$total_paginas = ceil($num_total_registros / $TAMANO_PAGINA); 

                    $db->select("menu_r","*"," where idr='".$_GET['idr']."'". " limit " . $inicio . "," . $TAMANO_PAGINA);  
...
?>
								...
								<tr>
									<td valign="top"><a href="#" title="<?php echo $row['nombre_l']?>" >
									  <?php echo $row['nombre_l']?>
									</a></td>
									<td><?php echo $row['url']?></td>
									
						          <td>
										<!-- Icons -->
										 <a class=Ntooltip href="<?= $_SERVER['PHP_SELF']?>?id=<?= $row['id_l']?>&amp;op=4&idr=<?php echo $idr ?>" title="Edit"><img src="imgs/pencil.png" alt="Edit" />
<span>
<?php $valor=variable(2,2); echo $valor[0]; ?> <?= $modulo; ?>
</span>                                         </a>
<a class=Ntooltip href="javascript:deletex('<?= $row['id_l']?>')" title="Delete"><img src="imgs/cross.png" alt="Delete" /><span>
