	...
	function pintar_categorias(&$arreglo, $nivel_cat){
		...
		foreach ($arreglo as $row){
			...
			if($row['nivel_categoria'] < $_GET['nivel']){
				$html .= '<a class=Ntooltip href="categoria.php?idr='.$_GET['idr'].'&nivel='.$_GET['nivel'].'&de='.$row['id_categoria'].'&numero='.$row['nivel_categoria'].'&op=4" title="">
				Crear categoria<img src="imgs/hammer_screwdriver (1).png" alt="Edit"/></a>';
			}
			$html .= '</td>
					</tr>';
			echo $html;
