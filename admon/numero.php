...
if ($op=="1"  ) { 
	...
	$sql = "UPDATE $t1 set nombre_numero='" . $_POST['nombre_numero'] . "'";
	$sql = $sql . " where id_numero= '" . $_POST['id'] . "'";
...
<? if ($op=="2") { 
	$sql = "INSERT INTO $t1(nombre_numero) values ";
	$sql = $sql . " ('" . $_POST['nombre_numero'] . "')";
...
<? if ($op=="3") { 
  ...
  $db->delete($t1,"where id_numero='" . $_GET['idb'] . "'");
  ?>
...
foreach ($cpadre as $row2) { 
                    ...
                    <td valign="top"><div align="center" class="campos_style">
                      <p class="titulos"> <img src="otros/iconos/32.png" width="64" height="64" />Administraci&oacute;n de numeros Bienvenido
                        ...
                        <table width="70%" border="1" cellpadding="0" cellspacing="0" bordercolor="#000000">
                          <tr>
                            <td><table width="100%" border="0" align="center" cellpadding="0" cellspacing="4" bordercolor="#666666" class="tabla">
                              <tr>
                                <th class="vinculo_dos">numero</th>
                                <th colspan="2" class="vinculo_dos">Opciones</th>
                                </tr>
                               <?
  $cpadre = array();
$db->select($t1,"*","order by  id_numero asc");
/*$db->last_query();*/
    while ($arraypadre = $db->fetch_array()) {
    $cpadre[] = $arraypadre; 
    }
...
?>
                                ...
                                <td width="22%" align="center" valign="middle" class="campos_style"><div align="center"><a href="<?= $_SERVER['PHP_SELF']?>?id=<?= $row['id_numero']?>&amp;op="><img src="otros/pequenos/action_refresh.gif" alt="" width="16" height="16" border="0" />Editar</a><a href="javascript:deletex('<?= $row['id_numero']?>')" class="campos_style"></a></div></td>
                                <td width="23%" align="center" valign="middle" class="campos_style"><a href="javascript:deletex('<?= $row['id_numero']?>')" class="campos_style"><img src="otros/pequenos/action_stop.gif" alt="" width="16" height="16" border="0" />Eliminar </a></td>
                              </tr>
  <?php } ?>
                      ...
                      <form action="<?= $_SERVER['PHP_SELF']?>?op=<?php echo $op ?>&idr=<?= $idr ?>" method="post" enctype="multipart/form-data" name="form1" id="form1" onSubmit="return formValidation();">
                        <div align="center" class="txt_titulo_vinculo"><img src="otros/iconos/31.png" width="64" height="64" />Numero
<? 
$db->select($t1,"*","where id_numero='" . $_GET['id'] . "'");
$row = $db->fetch_assoc();
?>
                        </div>
                        <table width="100%" border="2" cellpadding="0" cellspacing="0" bordercolor="#6CA223">
                          <tr>
                            <td><table align="center">
                              <tr valign="baseline">
                                <td align="right" nowrap="nowrap" class="campos_style"><div align="right">Numero :</div></td>
                                <td width="457"><div align="left">
                                    <input name="nombre_numero" type="text" class="campo" id="nombre_numero" value="<?= $row["nombre_numero"]?>" size="32" />
                                </div></td>
                              </tr>
                              <tr valign="baseline">
                                ...
                                <td><input name="submit" type="submit" class="rojos" value="Enviar informaci&oacute;n" />
                                  <input name="id" type="hidden" id="id" value="<?= $row["id_numero"]?>" /></td>
                              </tr>
                            </table></td>
                          </tr>
                        </table>
