...
$db->select("encuesta_pregunta","*","where id='" . $idr . "'");
$row = $db->fetch_assoc();

$papa=$row["nombre"];
$valor=variable(174,2); 
$modulo=$valor[0];
}
...
if ($op=="1") { 
	...
	$sql = "UPDATE $t1 SET id_sub='" . sql_seguro($_POST['idr']) . "'" . 
	        ", nombre= '" . sql_seguro($_POST['nombre'])  . "'" . 
			", fecha= '" . sql_seguro($fecha2) . "'" . 
...
<? if ($op=="2") { 
	$sql = "INSERT INTO $t1(id_sub, nombre, fecha) values " . 
...
$rowh = $db->fetch_assoc();
?> <?= $row["nombre"]?> -- <?php echo $papa ?> </span></h2>
              </div>
			  <div class="clear"></div>
			  <!-- End .clear -->
...
<p>
	  ...
	  <p>
									<input class="button" type="submit" value="<?php if($id){$valor=variable(2,2); echo $valor[0];} else{$valor=variable(3,2); echo $valor[0];}?> <?php echo $modulo ?>" /> <input name="id" type="hidden" id="id" value="<?= $row["id"]?>" />
...
$inicio = ($pagina - 1) * $TAMANO_PAGINA; 
                    ...
                    $db->select("encuesta_opcion","*","where id_sub='".$_GET['idr']."'");
                    $num_total_registros = $db->num_rows(); 
                    //calculo el total de p&aacute;ginas 
$total_paginas = ceil($num_total_registros / $TAMANO_PAGINA); 

                    $db->select("encuesta_opcion","*","where id_sub='".$_GET['idr']."'". " limit " . $inicio . "," . $TAMANO_PAGINA);
...
?>
								...
								<tr>
									<td valign="top"><a href="#" title="<?php echo $row['nombre']?>" >
									  <?php echo $row['nombre']?>
									</a></td>
						          ...
						          <td>
										<!-- Icons -->
										 <a class=Ntooltip href="<?= $_SERVER['PHP_SELF']?>?id=<?= $row['id']?>&amp;op=4&idr=<?php echo $idr ?>" title="Edit"><img src="imgs/pencil.png" alt="Edit" />
<span>
