...
{
    global $db;
    $db->select("palabra, idpa", "idpa.titulo_idpa, idpa.nombre_idpa, idpa.nota_idpa", "WHERE 
        palabra.id_palabra=idpa.palabra  and idpa.idioma='" . $idioma ."' AND palabra.codigo_palabra = '" . $codigo . 
        "' and idpa.idr='" . $grupo ."'");
    $rowt = $db->fetch_assoc();
    return $array = array($rowt['titulo_idpa'], $rowt['nota_idpa']);
}
...
* tiene un parametro de entrada que es opcional
...
{
    global $db;
    $db->select("rotan","*","WHERE id_rotan='" . $id_slider . "'");
    $row2 = $db->fetch_assoc($res2);
    $ancho = $row2['ancho'];
    $alto = $row2['alto'];

    $arr_slide = array();
    $db->select("slide", "*", "where id_rotan='" . $id_slider ."' ORDER BY rand()");
    while ($arraypadre = $db->fetch_array()) {
        $arr_slide[] = $arraypadre; 
    }
    $bit = "";
...
{
    global $db;
    $db->select("variable", "valor_variable, contenido_variable", "WHERE codigo_variable=".$id_variable." AND tipo_variable=".$idr);
    $row = $db->fetch_assoc();
    return $array = array($row['valor_variable'], $row['contenido_variable']);
}
...
{
    global $db;
    $db->select("textos_r", "titulo, contenido", "WHERE idt = '" . $idt . "'");
    $rowt = $db->fetch_assoc();
    return $array = array(utf8_encode($rowt['titulo']), utf8_encode($rowt['contenido']));
}
...
{
    global $db;
    $db->select("texto", "nombre_texto, descripcion", "WHERE id_texto = '" . $id_texto . "'");
    $rowt = $db->fetch_assoc();
    return $array = array(utf8_encode($rowt['nombre_texto']), utf8_encode($rowt['descripcion']));
}
...
function menu_recursivo($id = null)
{
    ...
    global $db;
    $db->select("categoria", "*", "{$de}");
    if($db->num_rows() >= 1){
        $menu .= "<ul>";
        $arr_categoria = array();
        while ($arraypadre = $db->fetch_array()) {
            $arr_categoria[] = $arraypadre; 
        }
        foreach ($arr_categoria as $row) {
            $menu .= "<li>{$row['nombre_categoria']}</li>";
            $menu .= menu_recursivo($row['id_categoria']);
        }
        $menu .= "</ul>";
...
{
    global $db;
    $db->select("categoria", "*", "WHERE id_categoria='{$id}'");
    $row2 = $db->fetch_assoc();
    return $row2['amigable_categoria'];
}
...
{
    global $db;
    $db->select("idmo", "*", "WHERE id_idmo='{$id}'");
    $row2 = $db->fetch_assoc();
    return limpiar_url($row2['nombre_idmo']);
}
...
{
    global $db;
    $db->select("matrix","*","WHERE id_matrix='{$id}'");
    $row2 = $db->fetch_assoc();
    return $row2['amigable_matrix'];
}
...
{
    global $db;
    $db->select("idima", "*", "WHERE id_imagen={$id} and id_idioma={$idio}");
    $rowt = $db->fetch_assoc();
    return $rowt['idma_imagen'];
}
...
{
    global $db;
    $db->select("palabra, idpa", "idpa.titulo_idpa, nota_idpa", "WHERE palabra.id_palabra=idpa.palabra AND 
        idpa.idioma='" . $idioma ."' AND palabra.codigo_palabra = '" . $codigo . "' AND 
        idpa.idr='" . $grupo ."'");
    $rowt = $db->fetch_assoc();
    return $rowt['titulo_idpa'];
}
...
function mostrar_nombre_tipo($id = null)
{
    $option = "";
    global $db;
    $db->select("vtalla", "*", "WHERE id_matrix={$id}");
    $row_banco = $db->fetch_assoc();
    $option = $row_banco['nombre_matrix'];
...
function get_stars($id,$tipo){
    ...
    $stars="";
    global $db;
    $db->select("votos", "SUM(valor) as valor,COUNT(id_comentario) as cont", "WHERE id_comentario =".$id." AND id_tipo=".$tipo);
    $row_RsV = $db->fetch_assoc();
    $totalRows_RsV = $db->num_rows();
    if($totalRows_RsV<>0){
        if($row_RsV['valor']<>"")$points=@round($row_RsV['valor']/$row_RsV['cont']);
        for($i=1;$i<=5;$i++){
            if($points<$i){
...
function get_valores_stars($id,$tipo){
    $valor=0;
    global $db;
    $db->select("votos", "SUM(valor) as valor,COUNT(id_comentario) as cont", "WHERE id_tipo=".$tipo." AND id_comentario =".$id);
    $row_RsV = $db->fetch_assoc();
    $totalRows_RsV = $db->num_rows();
    if($totalRows_RsV<>0){
        $valor=@round($row_RsV['valor']/$row_RsV['cont']);
...
/**/
function obtener_hijos_categoria(&$categoria){
    global $db;
    $db->select("categoria","*","WHERE de = '".$categoria['id_categoria']."'");
    /*$db->last_query();*/
    $cpadre = array();
    $arr_hijos = array();
    while ($arraypadre = $db->fetch_array()) {
    ...
    $categoria['hijos'] = $arr_hijos;
}

function obtener_categorias($modulo){
    $arr_categorias = array();
    global $db;
    $cpadre = array();
    $db->select("categoria","*","WHERE modulo = '".$modulo."' AND nivel_categoria = 1 ORDER BY ubica_categoria");
    /*$db->last_query();*/
    while ($arraypadre = $db->fetch_array()) {
        $cpadre[] = $arraypadre; 
    }
...
function borrar_categoria(&$categoria){
    if(sizeof($categoria['hijos']) > 0){
        foreach($categoria['hijos'] as $hijo){
            borrar_categoria($hijo);
        }
    }
    /*echo "<br>Borrar: categoria = ".$categoria['id_categoria']." - ".$categoria['nombre_categoria'];*/
    borrar_info_categoria($categoria);
}

function borrar_info_categoria(&$categoria){
    global $db;
    $cpadre = array();
    $db->select("matrix m, idmo i, modulo mo","m.*, mo.nombre_modulo","WHERE m.id_idmo = i.id_idmo AND i.modulo = mo.id_modulo AND m.id_categoria='".sql_seguro($categoria['id_categoria'])."'");
    /*$db->last_query();*/
    while ($arraypadre = $db->fetch_array()) {
        $cpadre[] = $arraypadre; 
    }
    foreach ($cpadre as $rows) {
        $carpeta = $rows['nombre_modulo'];     
        //imagen principal
        if($rows['img_matrix']!=""){
            if(file_exists("../imagenes/".$carpeta."/imagen1/".$rows['img_matrix'])){
                /*echo "<br>Borrar: ../imagenes/".$carpeta."/imagen1/".$rows['img_matrix'];*/
        ...
        if($rows['img1_matrix']!=""){
            if(file_exists("../imagenes/".$carpeta."/imagen2/".$rows['img1_matrix'])){
                /*echo "<br>Borrar: ../imagenes/".$carpeta."/imagen2/".$rows['img1_matrix'];*/
        ...
        if($rows['img2_matrix']!=""){
            if(file_exists("../imagenes/".$carpeta."/imagen3/".$rows['img2_matrix'])){
                /*echo "<br>Borrar: ../imagenes/".$carpeta."/imagen3/".$rows['img2_matrix'];*/
        ...
        if($rows['flash_matrix']!=""){
            if(file_exists("../imagenes/".$carpeta."/flash/".$rows['flash_matrix'])){
                /*echo "<br>Borrar: ../imagenes/".$carpeta."/flash/".$rows['flash_matrix'];*/
        ...
        if($rows['archivo_matrix']!=""){
            if(file_exists("../imagenes/".$carpeta."/archivo/".$rows['archivo_matrix'])){
                /*echo "<br>Borrar: ../imagenes/".$carpeta."/archivo/".$rows['archivo_matrix'];*/
        ...
        foreach ($cpadre as $row_secundarias) { 
            ...
            if($row_secundarias['foto']!=""){
                if(file_exists("../imagenes/".$carpeta."/secundaria/".$row_secundarias['foto'])){
                    /*echo "<br>Borrar: ../imagenes/".$carpeta."/secundaria/".$row_secundarias['foto'];*/
            ...
            /*echo "<br>Borrar: base de datos secundaria ".$row_secundarias['id'];*/
            $db->delete("secundaria","WHERE id='".sql_seguro($row_secundarias['id'])."'");
        }
        //elimino los productos o registros
        /*echo "<br>Borrar: base de datos matrix ".$rows['id_matrix'];*/
        $db->delete("matrix","WHERE id_matrix='".sql_seguro($rows['id_matrix'])."'");
        //termina while de secundaria   
    }
    /*borrar imagen de la categoria*/
    $db->select("categoria","*","WHERE id_categoria = '".$categoria['id_categoria']."'");
    $row = $db->fetch_assoc();

    /*borrar img*/
    if($row['img'] != ""){
        if(file_exists("../imagenes/categoria/".$row['img'])){
            /*echo "<br>Borrar: ../imagenes/categoria/".$row['img'];*/
            unlink("../imagenes/categoria/".$row['img']);
        }
    }

    /*borrar img1*/
    if($row['img1'] != ""){
        if(file_exists("../imagenes/categoria/c".$row['img1'])){
            /*echo "<br>Borrar: ../imagenes/categoria/c".$row['img1'];*/
            unlink("../imagenes/categoria/c".$row['img1']);
        }
    }
    /*echo "<br>Borrar: base de datos categoria ".$categoria['id_categoria'];*/
    $db->delete("categoria","WHERE id_categoria='".$categoria['id_categoria']."'");
}
?>